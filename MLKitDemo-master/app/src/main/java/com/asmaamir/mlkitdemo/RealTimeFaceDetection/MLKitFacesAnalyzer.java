...
public class MLKitFacesAnalyzer implements ImageAnalysis.Analyzer {
    ...
    MLKitFacesAnalyzer(TextureView tv, ImageView iv, CameraX.LensFacing lens, SmileRating smileRating) {
        this.tv = tv;
        this.iv = iv;
        this.lens = lens;
    ...
    private void processFaces(List<FirebaseVisionFace> faces) {
        for (FirebaseVisionFace face : faces) {
            ...
            int smileRatingValue=0;
            if (smileProbability > 0.8) {
                smileRatingValue = BaseRating.GREAT;
            } else if (smileProbability <= 0.8 && smileProbability > 0.6) {
                smileRatingValue = BaseRating.GOOD;
            } else if (smileProbability <= 0.6 && smileProbability > 0.4) {
                smileRatingValue = BaseRating.OKAY;
            } else if (smileProbability <= 0.4 && smileProbability > 0.2) {
                smileRatingValue= BaseRating.BAD;
